...
--#6 Display quantity of cars where cars brand is Chevrolet
--#6 Chevrolet brendinde kancha machine bar ekendigin chygarynyz
--#6 Показать количество автомобилей марки Chevrolet
select count(*) as Chevrolets from  cars where brand  = 'Chevrolet' ;
...
select   min(price)brand  from cars  ;
...
select  count(color) from  cars where  color = 'Pink';